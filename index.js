...
/**
 * If 'emit' has 2 arguments make emit with object as argument via template {name: 'eventName', data: 'objectThatPassedAsSecondArgumentInEmitFunc'}
 *
 * @static
 * @member {boolean}
 * @api public
 */
EventEmitter.useDataObject = false;
...
EventEmitter.prototype.emit = function emit(event, a1, a2, a3, a4, a5) {
	...
	if (listeners.fn) {
		...
		if (EventEmitter.useDataObject && len == 2) {
			listeners.fn.call(listeners.context, {name: evt, data: a1})
		} else {
			switch (len) {
				case 1:
					return EventEmitter.insertEventName
		...
		for (i = 0; i < length; i++) {
			...
			if (EventEmitter.useDataObject && len == 2) {
				listeners[i].fn.call(listeners.context, {name: evt, data: a1})
			} else {
				switch (len) {
					case 1:
						EventEmitter.insertEventName
